...
function hasNearbyPlayersForDiscount(game: ServerGameState, tile: HexTile, sourcePlayerId: string): boolean {
...
function createPowerOffers(game: ServerGameState, tile: HexTile, sourcePlayerId: string): void {
  ...
  for (const { playerId, maxPower, tileId } of nearbyPlayers) {
    ...
    // 이타르·타클론 제외: 1파워는 묻지 않고 자동 수락
    const autoAcceptOne = maxAffordable === 1 && targetPlayer.faction !== 'itars' && targetPlayer.faction !== 'taklons';
    if (autoAcceptOne) {
...
export function setupGameServer(httpServer: HTTPServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('start_game', ({ gameId }) => {
      ...
      // 턴 순서: 종족 선택 시 고른 순서(selectedTurnOrder)가 모두 있으면 그 순서로, 없으면 조인 순서
      const withOrder = Object.entries(game.players)
        .map(([id, p]) => ({ id, order: (p as any).selectedTurnOrder as number | undefined }))
        .filter(x => x.order !== undefined)
        .sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
      if (withOrder.length === Object.keys(game.players).length) {
        game.turnOrder = withOrder.map(x => x.id);
        log(`Start game: turn order set from selectedTurnOrder: ${game.turnOrder.join(', ')}`, 'game');
      }
      ...
      // 첫 차례 = 실제로 첫 배치해야 하는 사람(시퀀스 1번). 종족 미선택은 기본 2광산으로 시퀀스에 포함됨
          ...
          Object.entries(faction.startingTech).forEach(([track, level]) => {
            ...
            if (player.research) {
              ...
              if (lvl >= 1) {
                ...
                // Science and Economy are income-based, they will trigger in first income phase
              }
            }
          });
        }
    ...
    socket.on('place_starting_mine', ({ gameId, tileId, factionId }) => {
      ...
      if (game.currentPhase !== 'startingMines') { io.to(gameId).emit('game_error', { message: '지금은 시작 광산 배치 단계가 아닙니다.' }); return; }

      const playerId = socketToPlayerMap.get(socket.id);
      if (!playerId) { io.to(gameId).emit('game_error', { message: '플레이어 연결이 없습니다. 조작할 플레이어로 전환했는지 확인하세요.' }); return; }
      const player = game.players[playerId];
      if (!player) { io.to(gameId).emit('game_error', { message: '플레이어를 찾을 수 없습니다.' }); return; }
      ...
      if (!faction) {
        io.to(gameId).emit('game_error', { message: '종족을 먼저 선택하세요. (종족 선택 버튼 → 카드 클릭)' });
        return;
      }
      ...
      if (totalMinesPlaced >= snakingSequence.length) return;

      const expectedPlayerId = snakingSequence[totalMinesPlaced];
      // 항상 "지금 배치할 사람"으로 턴 표시 동기화 (종족을 나중에 고르면 start_game 시 턴이 잘못될 수 있음)
      ...
      if (playerId !== expectedPlayerId) {
        ...
        io.to(gameId).emit('game_error', { message: `지금은 다른 플레이어의 차례입니다. (한 컴퓨터 4인플이면 조작 플레이어가 자동 전환됩니다)` });
        clampPlayerResources(game);
        io.to(gameId).emit('game_updated', game);
        return;
      }

      if (player.startingMinesPlaced >= maxStartingMines) {
        io.to(gameId).emit('game_error', { message: '이미 시작 건물을 모두 배치했습니다.' });
        return;
      }

      const tile = game.map.find(t => t.id === tileId);
      if (!tile || tile.structure !== null) {
        io.to(gameId).emit('game_error', { message: '해당 타일에 배치할 수 없습니다.' });
        return;
      }

      if (tile.type !== faction.homePlanet) {
        io.to(gameId).emit('game_error', { message: `${faction.name}은(는) ${faction.homePlanet} 행성에만 배치할 수 있습니다.` });
        return;
      }
      ...
      if (newTotal >= snakingSequence.length) {
        ...
        // Find the index of next player in turnOrder
    ...
    socket.on('select_bonus_tile', ({ gameId, bonusTileId }) => {
      const game = games.get(gameId);
      if (!game || game.currentPhase !== 'bonusSelection') return;

      const playerId = socketToPlayerMap.get(socket.id);
      if (!playerId || game.pendingBonusSelection !== playerId) return;
      ...
      if (game.currentPlayerIndex < 0) {
        ...
        // 0라운드(시작 건물)에서는 파워 주고받지 않음. 파워 수신은 본게임에서 건물 지을 때만 생성됨.

        // 첫 플레이어의 턴 시작 상태 저장 (Reset 시 복원용)
    ...
    socket.on('use_bonus_action', ({ gameId, actionData }) => {
      const game = games.get(gameId);
      if (!game || game.currentPhase !== 'main') return;
      ...
      // 여기 도달하는 케이스 없음 (terraform_step, gaia_project, range_3 모두 위에서 return)
      // 액션 사용 후 자동으로 패스: 현재 보너스 타일을 반납하고 다음 라운드에 그 보너스 타일로 시작
      const currentBonusTileId = player.bonusTile;
      
      // 현재 보너스 타일을 다음 라운드 보너스 타일로 설정
      ...
      log(`Player ${player.name} used bonus action and auto-passed. Bonus tile ${currentBonusTileId} will be returned next round.`, 'game');
      ...
      if (allPassed) {
        ...
        // Update turn order based on passing sequence
        ...
        // Reset passed status for new round
      ...
      // 턴이 바뀐 직후 새 현재 플레이어의 턴 시작 상태 저장 (Reset 시 복원용)
    ...
    // 액션 시작 시점 상태 저장 (리셋 시 "마지막 액션 전"으로 복원용 — 턴 시작 상태는 덮어쓰지 않음)
    // Reset 시 파워(burn 포함)를 턴 시작으로 되돌리려면 turnStartState는 턴이 바뀔 때만 설정되어야 함.
    const saveActionStartState = (game: any, playerId: string) => {
      // turnStartState는 턴이 바뀔 때만 설정. 액션 시작 시 덮어쓰지 않음 → Reset 시 항상 턴 시작 시점(파워 포함)으로 복원
    };

    socket.on('build_mine', ({ gameId, tileId, useGaiaformer }) => {
      const game = games.get(gameId); if (!game || game.hasDoneMainAction) return;
      // 보너스 선택 단계에서는 광산 건설 불가
      ...
      // 우주선 연방 "광산 1개 무료 (Nav 무시)": 빈 행성이면 비용·Nav 없이 광산만 배치
      if (game.pendingSpaceshipFedMine?.playerId === playerId) {
        ...
        if (unbuildable.includes(tile.type) || tile.structure !== null) return;
        if (tile.type === 'asteroid') return;
      ...
      // 란티다 기생 광산: 다른 플레이어 건물이 있는 행성에 테라포밍 없이 1O 2C로 건설 (가이아/행성유형당 미포함, 연방·광산 이벤트 포함, 업그레이드 불가)
      if (player.faction === 'lantids' && tile.structure != null && tile.ownerId !== playerId && tile.ownerId != null && !tile.parasiticMine) {
        ...
        if (player.ore < mineOre || player.credits < mineCredits) return;
      ...
      if (unbuildableTypes.includes(tile.type)) return;
      
      // 소행성은 별도 처리
      if (tile.type === 'asteroid') {
        // Distance check (Nav+1은 범위에 직접 +1, 트왈라잇/보너스 +3, 글린 +2 적용)
        let baseRange = getRange(player.research.navigation || 0) + (player.navigationBonus || 0);
        if (player.tempRangeBonus) { baseRange += 3; player.tempRangeBonus = false; }
        if (player.rangeBonusActive) { baseRange += 3; player.rangeBonusActive = false; }
        if (player.gleensNavBonusActive) { baseRange += 2; player.gleensNavBonusActive = false; }
        ...
        if (getStructureCount(game, playerId, 'mine') >= BUILDING_LIMITS.mine) return;
        // 소행성은 가이아 포머만 사용 가능 (발타크: QIC 전환으로 잠긴 포머 제외)
      ...
      // Transdim+가이아포머(당장 건설 가능): TF2/보너스로 배치한 경우만 (이미 gaia로 성숙됨)
      // → 이 경로는 이제 gaia 타일에서 처리 (아래 gaia 분기)
      // 성숙된 가이아 타일(내 pendingGaiaformerTiles): 1O 2C로 건설, 가이아포머 회수
      ...
      // Transdim+가이아포머(같은 라운드 배치): 건설 불가여야 함 → pendingGaiaformerTiles에 없으면 여기 오지 않음
      if (tile.type === 'transdim') {
        if (!tile.hasGaiaformer) return;
        if (!player.pendingGaiaformerTiles?.includes(tileId)) return; // 일반 배치 직후에는 리스트에 없음 → 건설 불가
        if (getStructureCount(game, playerId, 'mine') >= BUILDING_LIMITS.mine) return;
        const mineOre = freeMine ? 0 : 1, mineCredits = freeMine ? 0 : 2;
        if (player.ore < mineOre || player.credits < mineCredits) return;
        player.ore -= mineOre;
      ...
      // Proto는 어떤 종족이든 건설 가능 (기존 제한 제거)
      // if (tile.type === 'proto' && faction.homePlanet !== 'proto') return;
      ...
      const mineOre = freeMine ? 0 : 1, mineCredits = freeMine ? 0 : 2;
      // 가이아 행성 (글린: 1 QIC 대신 1 광물 가능, 가이아 건설 시 +2 VP)
      if (tile.type === 'gaia') {
        const gaiaBaseQic = 1;
        const mineOre = freeMine ? 0 : 1, mineCredits = freeMine ? 0 : 2;
        const isGleens = player.faction === 'gleens';
        const useOreForGaia = isGleens && player.ore >= (mineOre + 1) && player.credits >= mineCredits && (player.qic ?? 0) >= neededQIC;
        const useQicForGaia = player.ore >= mineOre && player.credits >= mineCredits && (player.qic ?? 0) >= (neededQIC + gaiaBaseQic);
        if (!useOreForGaia && !useQicForGaia) return;
        if (useOreForGaia) {
          player.ore -= (mineOre + 1); player.credits -= mineCredits; player.qic -= neededQIC;
        } else {
          player.ore -= mineOre; player.credits -= mineCredits; player.qic -= (neededQIC + gaiaBaseQic);
        }
        terraformSteps = 0; // 가이아 행성은 테라포밍 단계가 없음
      } else {
        // Proto + 일반 행성 (확장: 모웨이드/팅커로이드 3+1, 다카니안 전부 1, 스페이스 자이언트 전부 2, proto=모웨이드/스자 0 그 외 3)
        ...
        if (player.ore < (terraformCost + mineOre) || player.credits < mineCredits || player.qic < neededQIC) return;
        player.ore -= (terraformCost + mineOre); player.credits -= mineCredits; player.qic -= neededQIC;
        player.pendingTerraformSteps = Math.max(0, pendingTerraformSteps - discountSteps);
      }

      if (getStructureCount(game, playerId, 'mine') >= BUILDING_LIMITS.mine) return;
      const geodensTypesBefore = getPlayerPlanetTypesForGeodens(game, playerId);
      // 다카니안 의회: 광산을 신규 섹터 또는 신규 외각 섹터에 지으면 1K 2C (배치 전에 판단). 건물 배치지 변경(예: 앰버스 스왑)에는 미적용.
      ...
      // 가이아 행성 건설 라운드 미션 점수 적용
      if (tile.type === 'gaia') {
        applyRoundMissionScore(game, playerId, 'build_gaia');
      }
      // 테라포밍 단계 점수 적용 (단계당 점수)
      if (terraformSteps > 0) {
        for (let i = 0; i < terraformSteps; i++) {
          applyRoundMissionScore(game, playerId, 'terraform_step');
          applyAdvancedTechTileEffect(game, playerId, 'terraform'); // 고급 타일: 테라포밍마다 2점
        }
      }
      // 고급 타일: 광산 지을 때마다 3점
      applyAdvancedTechTileEffect(game, playerId, 'build_mine');
      // QIC 사용 시 4점 (QIC를 사용한 경우)
      if (neededQIC > 0) {
    ...
    socket.on('upgrade_structure', ({ gameId, tileId, target }) => {
      ...
      if (tile.structure === 'lost_planet_mine') return; // 잊혀진 행성은 업그레이드 불가
      ...
      if (tile.structure === 'mine' && target === 'trading_station') {
        ...
        if (player.ore < 2 || player.credits < creditCost) return;
        player.ore -= 2; player.credits -= creditCost; tile.structure = 'trading_station'; game.hasDoneMainAction = true;
        // 라운드 미션 점수 적용
        applyRoundMissionScore(game, playerId!, 'build_trading_station');
        // 고급 타일: 교역소 지을 때마다 3점
        ...
        player.ore -= 3; player.credits -= 5; tile.structure = 'research_lab'; game.hasDoneMainAction = true;
        // 라운드 미션 점수 적용
        applyRoundMissionScore(game, playerId!, 'build_research_lab');
        addGameLog(game, playerId!, 'Upgraded to Research Lab', '3O, 5C', tileId);
        // 파워 교환 제안 생성
        createPowerOffers(game, tile, playerId!);
        // 연구소 건설 시 기술 타일 선택 요청
        ...
        if (player.faction === 'bescods') return; // 매안(Bescods)은 교역소→의회 불가, 연구소→의회만
        if (getStructureCount(game, playerId!, 'planetary_institute') >= BUILDING_LIMITS.planetary_institute) return;
        if (player.ore < 4 || player.credits < 6) return;
        player.ore -= 4; player.credits -= 6; tile.structure = 'planetary_institute'; game.hasDoneMainAction = true;
        // 하드쉬 할라: 의회 건설 시 프리 액션 3종 추가 (4C→1QIC, 4C→1K, 3C→1O)
        ...
        // 스페이스 자이언트: 의회 건설 시 기술 타일 1개 획득 (연구소/아카데미와 동일)
        ...
        if (academyTotal >= BUILDING_LIMITS.academy) return;
        const leftCount = getAcademyLeftCount(game, playerId!);
        const rightCount = getAcademyRightCount(game, playerId!);
        if (target === 'academy_left' && leftCount >= 1) return;
        if (target === 'academy_right' && rightCount >= 1) return;
        ...
        if (player.faction === 'bescods') return; // 매안(Bescods)은 연구소→아카데미 불가, 교역소→아카데미만
        const academyTotal = game.map.filter(t => t.ownerId === playerId && t.structure === 'academy').length;
        if (academyTotal >= BUILDING_LIMITS.academy) return;
        const leftCount = getAcademyLeftCount(game, playerId!);
        const rightCount = getAcademyRightCount(game, playerId!);
        ...
        if (idx !== -1 && selectedTrack) {
          ...
          if (canAdvance) {
            ...
            if (isRebellionGainTrack) {
              ...
              log(`Player ${player.name} gained tech tile ${techTileId} and advanced ${track} track to level ${levelNow}`, 'game');
            }
    ...
    socket.on('advance_tech', ({ gameId, trackId }) => {
      ...
      // 우주선 기술 타일 3개 중 하나 획득 후: 하단 풀 3개처럼 6개 트랙 중 원하는 트랙 1칸 무료 진행 (hasDoneMainAction 무관)
      ...
      // 고급 기술 타일 획득(덮기) 후: 아무 트랙 1칸 무료 진행 (우주선과 동일)
      ...
      // 4 Knowledge로 트랙 1칸 진행 (Eclipse 2K+3P는 eclipse_advance_track 사용). 발타크는 의회 없으면 Nav 불가.
      if (track === 'navigation' && !canBalTakAdvanceNavigation(game, playerId)) return;
      if (player.knowledge < 4 || player.research[track] >= 5) return;
      const newLevel = (player.research[track] ?? 0) + 1;
      if (newLevel === 5 && countGreenFederations(player) < 1) return;
    ...
    // 게임 로그 추가 헬퍼 함수
    function addGameLog(game: GaiaGameState, playerId: string, action: string, details?: string, tileId?: string) {
      if (!game.gameLog) {
        game.gameLog = [];
      }
      const player = game.players[playerId];
    ...
    // 라운드 미션 점수 적용 헬퍼 함수
    function applyRoundMissionScore(game: GaiaGameState, playerId: string, triggerType: string) {
    ...
    /** 6라운드 종료 시 최종미션 2개에 대해 1/2/3등(18/12/6점) 적용. 동점 시 해당 등수 점수 합산 후 인원수로 나눔. 0은 제외. */
    function applyFinalMissionScoring(game: GaiaGameState) {
    ...
    // RM7(New Sector/Bridge): 신규 섹터 또는 브릿지 섹터(11–18)에 광산을 새로 배치할 때만 3점.
    // 업그레이드(광산→교역소, 교역소→연구소/의회/아카데미 등)·배치지 변경(앰버스 스왑)에는 호출하지 않음.
    function qualifiesForNewSectorRoundMission(game: GaiaGameState, playerId: string, tileId: string, sector?: number): boolean {
    ...
    // 고급 기술 타일 자동 효과 (빌드/연구/테라포밍 시 점수)
    function applyAdvancedTechTileEffect(game: GaiaGameState, playerId: string, actionType: 'build_mine' | 'build_ts' | 'research' | 'terraform' | 'qic_action') {
      const player = game.players[playerId];
    ...
    // 고급 기술 타일 패스 효과
    function applyAdvancedTechTilePassEffect(game: GaiaGameState, playerId: string) {
      const player = game.players[playerId];
      if (!player || !player.techTiles) return;
    ...
    function applyTrackLevelBonus(game: GaiaGameState, playerId: string, player: PlayerState, track: ResearchTrack, newLevel: number) {
    ...
    function triggerIncomePhase(game: GaiaGameState) {
      ...
      if ((game as any).incomePhaseAppliedThisRound) {
        ...
        if (playersNeedingOrder.length > 0) {
          ...
          triggerIncomePhase(game); // 다음 대기 플레이어 처리
          return;
        }
      }
    ...
    socket.on('select_all_income_items', ({ gameId }) => {
      ...
      const tokenItems = items.filter(i => i.type === 'tokens');
      ...
      log(`Player ${player.name} auto-received all income: ${items.length} items`, 'game');
      clampPlayerResources(game); io.to(gameId).emit('game_updated', game);
    });
    ...
    socket.on('pass_round', ({ gameId, newBonusTileId }) => {
      const game = games.get(gameId); if (!game) return;
      // 보너스 선택 단계에서는 패스 불가 (보너스 선택만 가능)
      if (game.currentPhase !== 'main') return;
      const playerId = socketToPlayerMap.get(socket.id); if (!playerId) return;

      if (game.turnOrder[game.currentPlayerIndex] !== playerId) return;
      if (game.hasDoneMainAction) {
        log(`Player ${playerId} cannot pass after doing a main action. Use End Turn instead.`, 'game');
        return;
      }

      const player = game.players[playerId];

      // 6라운드에서는 보너스 타일 선택 없이 바로 패스 처리
      if (game.roundNumber === 6) {
        ...
        // 6라운드에서는 보너스 타일 반납 및 선택 없이 바로 패스
        player.hasPassed = true;
        if (!game.passingOrder.includes(playerId)) {
          game.passingOrder.push(playerId);
        }
        ...
        // 모든 플레이어가 패스했으면 게임 종료 + 최종미션 점수 적용 + 연구 트랙 보너스 + breakdown 정리
        if (Object.values(game.players).every(p => p.hasPassed)) {
          log(`All players passed. Game ended.`, 'game');
          applyFinalMissionScoring(game);
          // 연구 트랙 게임 종료 보너스: 3단계 4점, 4단계 8점, 5단계 12점
      ...
      // 1-5라운드: 보너스 타일 선택이 필수임 (클라이언트에서 이미 선택하도록 모달 표시)
      // newBonusTileId가 없으면 클라이언트에서 모달을 표시해야 함
      if (!newBonusTileId) {
        // 보너스 타일 선택 대기 상태로 전환 (클라이언트에서 모달 표시)
        game.pendingBonusSelection = playerId;
        clampPlayerResources(game); io.to(gameId).emit('game_updated', game);
        return;
      }

      // Calculate pass bonus from current bonus tile
      if (player.bonusTile) {
        const currentBonusTile = ALL_BONUS_TILES.find(t => t.id === player.bonusTile);
        if (currentBonusTile?.passBonus) {
          const playerStructures = game.map.filter(t => t.ownerId === playerId);
          ...
          switch (currentBonusTile.passBonus.type) {
            ...
            case 'gaia':
              ...
              break;
            case 'bridge_sector':
              // 외곽 브릿지 섹터 (sector 11-18)에 건물이 있는 섹터 수
              const bridgeSectors = new Set(
                playerStructures
                  .filter(t => t.sector >= 11 && t.sector <= 18)
                  .map(t => t.sector)
        ...
        // 고급 기술 타일 패스 효과 (1-5라운드)
        applyAdvancedTechTilePassEffect(game, playerId);

        // Return current bonus tile to available pool
      ...
      // Select new bonus tile (필수)
      const newTileIndex = game.availableBonusTiles.findIndex(t => t.id === newBonusTileId);
      if (newTileIndex === -1) {
        log(`Invalid bonus tile selection: ${newBonusTileId}`, 'error');
        return;
      }
      player.bonusTile = newBonusTileId;
      game.availableBonusTiles.splice(newTileIndex, 1);
      player.usedBonusAction = false; // Reset bonus action for new tile
      game.pendingBonusSelection = null; // 선택 완료
      ...
      if (Object.values(game.players).every(p => p.hasPassed)) {
        ...
        // 이번 라운드에 배치한 가이아포머 타일 성숙: transdim → gaia, 건설 가능 목록에 추가
        Object.entries(game.players).forEach(([pId, player]) => {
          ...
          placed.forEach(tileId => {
            ...
            if (t && t.type === 'transdim' && t.hasGaiaformer && !t.structure) {
              t.type = 'gaia';
              // 가이아포머는 그대로 둠(설치한 플레이어만 건설 가능 확인용), 광산 짓을 때 회수
