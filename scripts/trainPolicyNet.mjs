...
const MAIN=['Built Mine','Advanced Research','Upgraded to Trading Station','Upgraded to Research Lab','Federation','Gained Tech Tile','Entered Ship','Academy','Power Action','Used Tech Action','Placed Gaiaformer'];
...
const labelOf=a=>MAINSET.has(a)?a:null;
...
for(const f of files){let g;try{g=JSON.parse(fs.readFileSync(dir+'/'+f,'utf8'))}catch{continue}
  ...
  for(const e of g.actionJournal){const pid=e.playerId,act=e.action||'',tid=e.tileId,lab=labelOf(act);
    if(lab&&e.playerBefore){const mine=[...owner.entries()].filter(([,o])=>o===pid).map(([id])=>geom.get(id)).filter(Boolean);
      ...
      samplesAll.push({f:[...scalarFeat(e),en.mines/12,en.builds/18,en.ships/3,en.feds/3,en.tech/8,mine.length/12,types.size/7,sectors.size/8,Math.min(nShip,9)/9,Math.min(nProto,9)/9,mine.length?adj/mine.length:0],y:MAIN.indexOf(lab)});}
...
const L=MAIN.length,D=samplesAll[0].f.length+1;
console.log('학습 샘플 '+samplesAll.length+', 클래스 '+L+', 차원 '+(D-1));
const W=Array.from({length:L},()=>new Float64Array(D));
const xs=samplesAll.map(s=>[...s.f,1]),ys=samplesAll.map(s=>s.y);
...
const out={version:1,labels:MAIN,featDim:D-1,featSpec:'scalar13+eng5+map6',W:W.map(w=>Array.from(w))};
