// trainPolicyNetMLP.mjs + 종족 one-hot(18). featDim 26→44. 별도 파일 저장(baseline 비교용).
// 2026-06-27 종족피처 실험이 2147샘플(120/종족)로 과적합→기각. 이번 9055샘플(500/종족)로 재시도.
import fs from 'fs';
...
const FACTIONS=['terran','lantids','hadsch_hallas','ivits','geodens','bal_tak','xenos','gleens','taklons','ambas','bescods','firaks','itars','nevlas','moweyip','space_giants','tinkeroids','darkanians'];
...
const facOneHot=(fac)=>FACTIONS.map(fc=>fc===fac?1:0);
...
for(const f of files){let g;try{g=JSON.parse(fs.readFileSync(dir+'/'+f,'utf8'))}catch{continue}
  ...
  for(const e of g.actionJournal){const pid=e.playerId,act=e.action||'',tid=e.tileId,lab=labelOf(act,e.details);
    if(lab&&e.playerBefore&&e.faction){const mine=[...owner.entries()].filter(([,o])=>o===pid).map(([id])=>geom.get(id)).filter(Boolean);
      ...
      samplesAll.push({f:[...scalarFeat(e),en.mines/12,en.builds/18,en.ships/3,en.feds/3,en.tech/8,mine.length/12,types.size/7,sectors.size/8,Math.min(nShip,9)/9,Math.min(nProto,9)/9,mine.length?adj/mine.length:0,...facOneHot(e.faction)],lab});}
...
const L=LABELS.length, D=samplesAll[0].f.length;
const X=samplesAll.map(s=>s.f), Y=samplesAll.map(s=>LABELS.indexOf(s.lab));
console.log('학습 샘플 '+samplesAll.length+', 클래스 '+L+', 입력차원 '+D+' (종족 one-hot 포함)');
  ...
  for(let ep=0;ep<epochs;ep++){
    ...
    for(let bs=0;bs<order.length;bs+=batch){
      ...
      for(let bi=bs;bi<bend;bi++){const i=order[bi],x=X[i],y=Y[i];
        ...
        for(let c=0;c<L;c++){const g=dl2[c];gb2[c]+=g;const w=net.W2[c],gw=gW2[c];
...
const configs=[
  ...
  {H:96,lr:0.002,epochs:800,l2:2e-3,batch:64},
];
let best=null;
for(const cfg of configs){
  ...
  console.log(`H=${cfg.H} lr=${cfg.lr} l2=${cfg.l2} | train t1 ${(tr.top1*100).toFixed(1)}% | val t1 ${(va.top1*100).toFixed(1)}% t3 ${(va.top3*100).toFixed(1)}% | gap ${gap.toFixed(1)}pt${gap>12?' (overfit)':''}`);
  if(!best||va.top1>best.va.top1){best={cfg,net,tr,va};}
}
const {cfg,net,tr,va}=best;
console.log(`\n=== BEST(종족): H=${cfg.H} l2=${cfg.l2} | val top1 ${(va.top1*100).toFixed(1)}% top3 ${(va.top3*100).toFixed(1)}% | 베이스라인(무종족) val top1 30.4% ===`);
const out={version:2,arch:'mlp',activation:'relu',labels:LABELS,featDim:D,hidden:cfg.H,featSpec:'scalar13+eng5+map6+fac18',factions:FACTIONS,
  W1:net.W1.map(r=>Array.from(r)),b1:Array.from(net.b1),W2:net.W2.map(r=>Array.from(r)),b2:Array.from(net.b2)};
fs.writeFileSync('server/ai/policyNetMLPfac.json',JSON.stringify(out));
console.log('저장: server/ai/policyNetMLPfac.json');
