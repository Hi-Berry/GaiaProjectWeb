...
const FAQ_ITEMS: { q: string; a: string }[] = [
  { q: '프리액션(자원 변환)은 어떻게 하나요?', a: '상태창의 "FA OFF" 버튼을 누르면 "FA ON"이 됩니다. 이때 자원(O·K·Q·C)·파워 그릇 숫자를 클릭하면 즉시 변환돼요. "FA ON" 상태에서 버튼을 다시 누르면 이번 턴에 한 프리액션이 모두 취소(Undo All)됩니다.' },
  { q: '연방은 어떻게 만드나요?', a: '우측 "연방" 버튼으로 연방 모드에 들어간 뒤, 내 건물들을 잇는 빈 칸(우주)을 클릭하면 위성이 놓이며 연방이 형성됩니다. 파워 합이 7 이상 되도록 빈 칸만 클릭해도 연방이 만들어져요.' },
  { q: '기술 타일은 어떻게 쓰나요?', a: '대부분은 자동입니다 — 매 라운드 수입, 즉시 VP, "행동할 때마다 +VP" 같은 효과는 알아서 적용돼요. 직접 쓰는 건 "ACT" 계열(3K·3O·1Q+5C·4P)뿐이고, 상태창(내 정보) 상세를 열어 그 타일 이미지를 클릭하면 매 라운드 1회 사용됩니다(내 턴·메인 액션 전·미사용일 때, 앰버 테두리로 표시). 타일 획득은 연구소·아카데미 건설 시 또는 연구 트랙 진행 등으로.' },
  ...
  { q: '발타크: 가이아포머가 QIC로 자동 변환되나요?', a: '아니요, 자동이 아닙니다. 프리액션으로 직접 변환합니다 — "FA ON"(또는 Free Actions 창)에서 "1 포머 → 1 QIC"를 실행하세요. 변환에 쓴 포머는 다음 라운드 시작까지 잠겨(가이아 토큰으로 표기) 그 라운드엔 재사용할 수 없습니다.' },
];
