...
export function GameLobby({ game, gameId, playerId, onStartGame, onLeave, onAddPlayer, onSwitchPlayer }: GameLobbyProps) {
              ...
              {Array.from({ length: maxPlayers - playerCount }).map((_, i) => (
                <div
                  ...
                  className={`flex items-center gap-2 p-3 rounded-lg border border-dashed ${
                    i === 0 && isHost && onAddPlayer ? 'border-primary/50 bg-primary/5' : 'border-muted-foreground/30'
                  }`}
                >
