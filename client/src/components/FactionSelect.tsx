      ...
      <div className="text-center space-y-1">
        ...
        <div className="flex justify-center mt-2">
          <Button
            variant="outline"
            size="sm"
            className="text-amber-400 border-amber-500/50 bg-amber-950/20 hover:bg-amber-900/40"
            onClick={() => {
              const availableOrders = availableTurnOrders.filter(o => !takenTurnOrders.includes(o));
              if (availableOrders.length === 0) return;
              const randomOrder = availableOrders[Math.floor(Math.random() * availableOrders.length)];

              const availableFactionIds = FACTIONS.filter(f => {
                const isFactionTaken = takenFactions.includes(f.id);
                const isColorTaken = Object.entries(game.players).some(([id, p]) => {
                  if (id === playerId || !p.faction) return false;
                  const otherFaction = FACTIONS.find(of => of.id === p.faction);
                  return otherFaction?.color === f.color;
                });
                return !isFactionTaken && !isColorTaken;
              }).map(f => f.id);

              if (availableFactionIds.length > 0) {
                const randomFactionId = availableFactionIds[Math.floor(Math.random() * availableFactionIds.length)];
                onSelectFaction(randomFactionId, randomOrder);
                setSelectedTurnOrder(randomOrder);
              }
            }}
          >
            Random Selection
