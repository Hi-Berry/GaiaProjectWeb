...
export function GameBoard({
  ...
  /** [사용자 2026-08-23] 높이는 내용만큼 자동, 폭만 사용자가 정한다.
   *  (예전엔 높이도 저장·조절했는데 우주선 칸처럼 내용이 긴 타일에서 스크롤이 남았다.) */
  const [detailWidth, setDetailWidth] = useState<number>(() => {
    const vw = typeof window !== 'undefined' ? window.innerWidth : 400;
    try {
      const v = localStorage.getItem('gaia-tile-detail-w');
      if (v && Number(v) >= DETAIL_MIN_W) return Math.min(Number(v), vw - 8);
      const old = localStorage.getItem('gaia-tile-detail-size'); // 이전 형식({w,h})에서 폭만 이어받는다
      if (old) { const o = JSON.parse(old); if (o?.w >= DETAIL_MIN_W) return Math.min(o.w, vw - 8); }
    } catch { /* noop */ }
    return Math.min(300, vw - 16);
  });
  ...
  const detailWidthRef = useRef(detailWidth); detailWidthRef.current = detailWidth;
  ...
  const detailStartResize = useCallback((e: React.PointerEvent) => {
    ...
    const sx = e.clientX;
    const sw = detailWidthRef.current;
    const onMove = (ev: PointerEvent) => setDetailWidth(
      Math.max(DETAIL_MIN_W, Math.min(window.innerWidth - 8, sw + (ev.clientX - sx))),
    );
    const onUp = () => {
      window.removeEventListener('pointermove', onMove);
      window.removeEventListener('pointerup', onUp);
      document.body.style.userSelect = '';
      try { localStorage.setItem('gaia-tile-detail-w', String(detailWidthRef.current)); } catch { /* noop */ }
    };
    document.body.style.userSelect = 'none';
    window.addEventListener('pointermove', onMove);
    window.addEventListener('pointerup', onUp);
  ...
  // [사용자] 폰에서 맵을 '폭 맞춤 + 가로 중앙 + 윗공간 배치'로 자동 배치한다(기존: 하단에 작게 뜨던 문제).
  //   - 초기 1회는 모드 상관없이 배치(맵줌 모드는 이후 사용자가 자유롭게 조정 → 기본값일 때만 덮음).
  //   - 전체확대(page pinch) 모드는 맵을 못 만지므로 진입·리사이즈마다 재배치.
  //   맵 콘텐츠(<g>) 실제 픽셀을 재서 현재 줌 제거→자연 크기로 환산 후 목표 줌/팬 산출.
  const didInitialFitRef = useRef(false);
  useEffect(() => {
    if (!isMobileViewport) return; // 데스크톱은 기존 기본값 유지
    ...
    const fit = (): boolean => {
      ...
      if (naturalW < 5 || naturalH < 5) return false; // 아직 렌더 전
    ...
    // 맵줌 모드: 최초 1회만, 그리고 사용자가 아직 안 만진(기본값) 상태일 때만 초기배치
    const atDefaults = Math.abs(zoomRef.current - 1) < 0.01 && Math.abs(pan.x) < 1 && Math.abs(pan.y) < 1;
    if (!didInitialFitRef.current && atDefaults) {
        ...
        >
          <HexGrid width={1200} height={1000} viewBox="-50 -50 250 250">
            ...
            <g id="sector-backgrounds-layer" style={{ filter: 'saturate(0.82) brightness(0.9)' }}>
              ...
              let spinTransform = '';
              if (slotTile.spin && slotTile.sector >= 11 && slotTile.sector !== 90) {
                const trio = game.map.filter(t => t.sector === slotTile.sector);
                if (trio.length === 3) {
                  const gx = trio.reduce((a, t) => a + HEX_SIZE * SQRT3 * (t.q + t.r / 2), 0) / 3;
                  const gy = trio.reduce((a, t) => a + HEX_SIZE * 1.5 * t.r, 0) / 3;
                  spinTransform = `rotate(${slotTile.spin * 120}, ${gx}, ${gy}) `;
                }
              }
                ...
                // 면(side)은 타일 상태를 따른다(2026-09-19 면·위치·제자리회전 랜덤). 구버전 저장분(side 없음)은 예전 고정 면.
                  ...
                  const prefix = slotTile.side ? `Map_${slotTile.side}` : (legacySideO ? 'Map_O' : 'Map_B');
                ...
                return (
                  ...
                  >
                    ...
                    {hoveredPlayerId && (() => {
                      ...
                      // [사용자] 연방/미연방 건물 모두 굵은 실선+강한 글로우로 동일하게 강조(색만 구분: 연방=금색, 미연방=파란/시안).
                      return (
                        <g className="pointer-events-none">
                          <circle
                            r="4.3"
                            ...
                            className="animate-pulse"
                            style={{
                              filter: `drop-shadow(0 0 7px ${highlightColor})`,
                              opacity: 1
                            }}
                          />
        ...
        <div className="absolute top-4 right-4 flex items-start gap-2 z-30 pointer-events-none [&>*]:pointer-events-auto">
          ...
          {(() => {
            ...
            return isFederationMode ? (
              // '연방 구현' 버튼 자리(우상단)에 그대로 표시 — 하단 고정 시 채팅창에 가려지는 문제 해소
              <div className="p-2 bg-black/85 backdrop-blur-md border border-sky-500/40 rounded-xl flex flex-col gap-1.5 w-64 shadow-2xl pointer-events-auto">
        ...
        selectedTile && !isFederationMode && (
          <div
            ...
            style={detailFloating
              ? (() => {
                const top = detailPos?.y ?? Math.max(8, Math.round(cssHeight * 0.06));
                return {
                  width: detailWidth,
                  // 높이는 내용만큼(auto). 화면 아래로 넘치는 것만 막고, 그때는 본문이 스크롤된다.
                  maxHeight: `calc(100dvh - ${top + 8}px)`,
                  left: detailPos?.x ?? 8,
                  top,
                };
              })()
          ...
          >
            ...
            <div
              ...
              style={(() => {
                ...
                if (detailFloating) {
                  // 창 모드는 높이를 내용에 맡긴다 — height를 %로 강제하면 부모가 auto일 때 계산이 성립하지 않는다.
                  return { width: 256, zoom: detailWidth / 256 } as CSSProperties;
                }
                const w = isMobileViewport ? mobilePanelWidth : 0;
                    ...
                    {currentPlayer.pendingGaiaformerTiles?.includes(selectedTile.id) ? (
                      <>
                        ...
                        <Button
                          ...
                          disabled={(game.hasDoneMainAction && (!game.pendingShipTechMine || game.pendingShipTechMine.playerId !== playerId) && (!game.pendingSpaceshipFedMine || game.pendingSpaceshipFedMine.playerId !== playerId)) || (game.turnOrder[game.currentPlayerIndex] !== playerId && game.pendingShipTechMine?.playerId !== playerId)}
                          onClick={() => {
                ...
                {currentPlayer?.faction === 'lantids' && selectedTile.structure != null && selectedTile.ownerId !== playerId && selectedTile.ownerId != null && !selectedTile.parasiticMine && onBuildMine && (() => {
                  ...
                  return (
                    <Button
                      ...
                      disabled={(game.hasDoneMainAction && (!game.pendingShipTechMine || game.pendingShipTechMine.playerId !== playerId) && (!game.pendingSpaceshipFedMine || game.pendingSpaceshipFedMine.playerId !== playerId)) || (game.turnOrder[game.currentPlayerIndex] !== playerId && game.pendingShipTechMine?.playerId !== playerId) || !canAfford}
                      onClick={() => {
                        onBuildMine(selectedTile.id);
                        setSelectedTile(null);
                      }}
            ...
            {detailFloating && (
              ...
              <div
                ...
                title="폭 조절"
                className="absolute top-0 bottom-0 right-0 w-3 cursor-ew-resize touch-none select-none flex items-center justify-center"
              >
                <div className="h-8 w-0.5 rounded bg-white/25" />
              </div>
            )}
          </div>
        )
